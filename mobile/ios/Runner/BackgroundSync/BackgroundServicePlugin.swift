...
class BackgroundServicePlugin: NSObject, FlutterPlugin {
  ...
  // e -l objc -- (void)[[BGTaskScheduler sharedScheduler] _simulateLaunchForTaskWithIdentifier:@"com.pizcloud.photos.backgroundFetch"]
  // or
  // e -l objc -- (void)[[BGTaskScheduler sharedScheduler] _simulateLaunchForTaskWithIdentifier:@"com.pizcloud.photos.backgroundProcessing"]
  ...
  public static let backgroundProcessingTaskID = "com.pizcloud.photos.backgroundProcessing"
  ...
  func handleDigestFiles(call: FlutterMethodCall, result: @escaping FlutterResult) {
    ...
    DispatchQueue.global(qos: .background).async {
      ...
      for i in (0..<args.count) {
        do {
          guard let file = FileHandle(forReadingAtPath: args[i]) else {
            throw DigestError.NoFileHandle
          }
          ...
          print(
            "Cannot calculate the digest of the file \(args[i]) due to \(error.localizedDescription)"
          )
        }
      }
  ...
  static func scheduleBackgroundFetch() {
    ...
    let backgroundFetch = BGAppRefreshTaskRequest(
      identifier: BackgroundServicePlugin.backgroundFetchTaskID)
  ...
  static func scheduleBackgroundSync() {
    let backgroundProcessing = BGProcessingTaskRequest(
      identifier: BackgroundServicePlugin.backgroundProcessingTaskID)
